...
int EthernetClient::connect(const char* host, uint16_t port, unsigned long timeout) {
	...
	while (!ip.addr) {
		delay(10);//Wasn't changed because it is not used
	}
...
int EthernetClient::connect(IPAddress ip, uint16_t port, unsigned long timeout) {
	...
	unsigned long then = micros();//Changed by Alejandro, was millis()

	while (!_connected) {
		unsigned long now = micros();//Changed by Alejandro, was millis()
		//delay(10);Commented by Alejandro
		if (now - then > timeout) {
	...
	while (i < size) {
		...
		if (err != ERR_MEM) {
			...
			if (!stuffed_buffer) {
				...
				ResetWatchDogTimer();//Written by Alejandro
				delayMicroseconds(800);//Written by Alejandro
				ResetWatchDogTimer();//Written by Alejandro
			}
		}
	}
